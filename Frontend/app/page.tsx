...
function RangeField({label,min,max,value,unit,onChange}:{label:string;min:number;max:number;value:number;unit:string;onChange:(v:number)=>void}){const pct=((value-min)/(max-min))*100;return <div className="range-field"><div className="range-head"><span>{label}</span><strong>{value} <small>{unit}</small></strong></div><div className="range-track"><span style={{width:`${pct}%`}}/><input aria-label={label} type="range" min={min} max={max} value={value} onChange={e=>onChange(Number(e.target.value))}/></div><div className="range-scale"><span>{min}</span><span>{Math.round((min+max)/2)}</span><span>{max}</span></div></div>}
        ...
        {mode === "reset" ? (
          <div className="auth-fields">
            ...
            {resetStep === "request" && <>
              ...
              <button type="button" className="primary-button wide" disabled={busy} onClick={requestOtp}>{busy ? <><LoadingDots label={t("auth.sending")}/> {t("auth.sendingCode")}</> : t("auth.sendCode")}</button>
            </>}
            {resetStep === "verify" && <>
              {resetInfo && <div className="info-box">{resetInfo}</div>}
              <label>{t("auth.verificationCode")}<input value={resetOtp} onChange={e=>setResetOtp(e.target.value)} placeholder={t("auth.codePlaceholder")} maxLength={6}/></label>
              ...
              {error && <div className="error-box">{error}</div>}
              <button type="button" className="primary-button wide" disabled={busy} onClick={confirmReset}>{busy ? <><LoadingDots label={t("auth.updating")}/> {t("auth.updatingCode")}</> : t("auth.resetPasswordCta")}</button>
...
function ProgressPage({student,completed,grades,courses,onNavigate,history,onModal}:{student:Student;completed:string[];grades:Record<string,string>;courses:Course[];onNavigate:(v:View)=>void;history:HistoryItem[];onModal:(t:string,b:React.ReactNode)=>void}) {
  ...
  return (
    <div className="page">
      ...
      <div className="progress-hero"><div><small>{t("progress.degreeProgress")}</small><h2>{t("progress.creditsOf120",{credits})}</h2><Progress value={pct}/><p>{t("progress.gradTargetPct",{pct:Math.round(pct)})}</p></div><div className="progress-ring"><strong>{student.gpa.toFixed(2)}</strong><span>{t("progress.gpa")}</span></div></div>
