...
export default function Home(){
  ...
  useEffect(()=>{(async()=>{try{setDataLoading(true);const id=sessionStorage.getItem("masar_student_id");const coursePromise=loadCourses();if(id){try{const d=await loadStudent(id);setView("catalog");const done=(d.completed_courses||[]).map(c=>typeof c === "string" ? normalizeCourseCode(c) : normalizeCourseCode(c.course_code));await coursePromise;setCheckingSession(false);setDataLoading(false);void Promise.all([refreshPlans(d.student,done),loadHistory(id)]).catch(e=>setError(e instanceof Error?e.message:t("err.couldNotLoadDashboard")));}catch{sessionStorage.removeItem("masar_student_id");sessionStorage.removeItem("masar_access_token");await coursePromise}}else{await coursePromise}}catch(e){setError(e instanceof Error?e.message:t("err.unableToInitialize"))}finally{setDataLoading(false);setCheckingSession(false)}})()},[]);
  const authenticated=(s:Student,initialCompleted:string[]=[],initialPlans?:Plans,initialGrades:Record<string,string>={})=>{
    ...
    setError(""); setPlans(initialPlans||{}); setHistory([]); setStudent(s); setCompleted(cleanCompleted); setCompletedGrades(initialGrades||{}); setSelectedCourses([]); setView("catalog"); setWelcome(true);
