...
function localizeRecommendationReason(reason: string | undefined, lang: Lang, fallback: string) {
  ...
  if (parts.length) return `موصى به: ${parts.join("؛ ")}.`;
  // Never leak an English AI explanation into Arabic mode. The backend
  // validates Arabic output too, but this is a final UI-side safety net.
  if (/[A-Za-z]{3,}/.test(reason) && !/[\u0600-\u06FF]/.test(reason)) {
    return fallback;
  }
...
export function RecommendationPage({student,plan,planType="balanced",completedCourses=[],onNavigate,onModal,loading}:{student:Student;plan?:Plan;planType?:string;completedCourses?:string[];onNavigate:(v:View)=>void;onModal:(t:string,b:React.ReactNode)=>void;loading?:boolean}) {
  const {t, lang} = useLanguage();
  const [explaining, setExplaining] = useState(false);
  const recs=plan?.recommendations||[];
  const explainPlan = async () => {
    if (!plan || explaining) return;
    setExplaining(true);
    try {
      const data = await api<{success:boolean;source:"ai"|"rule";explanation:{summary:string;profile_fit:string;workload:string;exclusions:string}}>("/recommend/explain-plan", {
        method: "POST",
        body: JSON.stringify({
          student_id: student.student_id,
          full_name: student.full_name,
          academic_major: student.academic_major || null,
          academic_career_goals: student.academic_career_goals || null,
          gpa: Number(student.gpa),
          math_confidence: Number(student.math_confidence),
          programming_confidence: Number(student.programming_confidence),
          workload_tolerance: Number(student.workload_tolerance),
...
export function Profile({student,courses,completed,initialGrades,onSaved,onModal,onLogout}:{student:Student;courses:Course[];completed:string[];initialGrades:Record<string,string>;onSaved:(s:Student,done:string[],grades:Record<string,string>)=>void|Promise<void>;onModal:(t:string,b:React.ReactNode)=>void;onLogout:()=>void}) {
  const {t} = useLanguage();
  const [form,setForm]=useState({...student});const [done,setDone]=useState(completed);const [grades,setGrades]=useState<Record<string,string>>({});const [password,setPassword]=useState("");const [q,setQ]=useState("");const [busy,setBusy]=useState(false);const [error,setError]=useState("");const fileRef=useRef<HTMLInputElement>(null);const [gpaText,setGpaText]=useState(String(student.gpa??0));
  const filtered=courses.filter(c=>`${c.course_code} ${c.course_name}`.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>Number(done.includes(b.course_code))-Number(done.includes(a.course_code)));useEffect(()=>{setForm({...student});setDone(completed);setGrades(initialGrades||{});setGpaText(String(student.gpa??0))},[student,completed,initialGrades]);
